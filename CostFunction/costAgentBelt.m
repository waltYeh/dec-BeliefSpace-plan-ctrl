function c = costAgentBelt(D, idx, b, u, horizon, stDim)
...
%         RowIdx = ismember(D.Edges.EndNodes, [j,idx],'rows');
        formation_error = x_idx-x;
        sc = sc + L*10*q_formation*(formation_error'*formation_error);
    end
    ...
    for j_nid = 1:length(nid)
        ...
        sc = sc + 0.05*q_formation*(formation_error'*formation_error);
    end
