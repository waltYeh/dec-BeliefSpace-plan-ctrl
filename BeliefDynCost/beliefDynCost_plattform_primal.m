function [g,c,gb,gu,gbb,gbu,guu,c_bi,c_ui,c_bi_bi,c_bi_ui,c_ui_ui,c_ui_uj]...
    =beliefDynCost_plattform_primal(D,idx,b,u,uC_lambda, rho,horizonSteps,full_DDP,...
...
if nargout < 5
    for j = incoming_nbrs_idces
        ...
        uj = squeeze(u{j});
        try
            uC_lambda_j = squeeze(uC_lambda{j});
        catch ME
            keyboard
        end
...
%             b_formation(j,:,:) = [avr_x;avr_P(:)];
    ...
    u_C_lambda_formation(idx,:,:) = uC_lambda{idx};
    g{idx} = belief_dyns{idx}(b{idx}, u{idx});
%     c = costAssistingRobot(b{idx}, u{idx}, horizonSteps, motionModel.stDim,components_amount);
    
    c = costAgentFormation_primal(D, idx,b_formation, u_formation,u_C_lambda_formation, ...
