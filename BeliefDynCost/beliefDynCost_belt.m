function [g,c,gb,gu,gbb,gbu,guu,c_bi,c_ui,c_bi_bi,c_bi_ui,c_ui_ui,c_ui_uj] ...
    = beliefDynCost_belt(D,idx,b,u,horizonSteps,full_DDP,motionModel,obsModel,belief_dyns)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     c = costAssistingRobot(b{idx}, u{idx}, horizonSteps, motionModel.stDim,components_amount);
    ...
    c = costAgentBelt(D, idx,b_formation, u_formation, horizonSteps, motionModel.stDim);
else
...
%             b_formation(j,:,:) = [avr_x;avr_P(:)];
    ...
    b_formation(idx,:,:) = b{idx};
    u_formation(idx,:,:) = u{idx};
    xu_cost = @(xu) costAgentBelt(D,idx,xu(:,ib,:),xu(:,iu_begin:end,:),horizonSteps,motionModel.stDim);    
%     J       = 
...
%     cu(idx,:,:)      = J(1,iu_begin:end,:);
    ...
    xu_cost_nocc = @(xu) costAgentBelt(D,idx,xu(:,ib,:),xu(:,iu_begin:end,:),horizonSteps,motionModel.stDim);
