...
mu_save = cell(agents_amount,max_components_amount);
sig_save = cell(agents_amount,max_components_amount);
weight_save = cell(agents_amount,max_components_amount);
x_save = [];
x_true = [];
mu = cell(agents_amount,max_components_amount);
sig = cell(agents_amount,max_components_amount);
...
for i = 1:agents_amount
    for i_comp=1:agents{1}.components_amount
        mu_save{i,i_comp} = mu{i,i_comp};
        sig_save{i,i_comp} = sig{i,i_comp}(:);
...
%     processNoise = motionModel.generateProcessNoise(x_true,u);
    ...
    for i = 1:1%agents_amount
        [b_next_i,mu_i,sig_i,weight_i] = agents{i}.getNextEstimation(b{i},u,z);
        b{i}=b_next_i;
        for i_comp=1:agents{i}.components_amount
            mu{i,i_comp}=mu_i{i_comp};
            sig{i,i_comp}=sig_i{i_comp};
