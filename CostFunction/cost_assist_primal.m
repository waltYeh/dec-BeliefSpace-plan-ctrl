function c = cost_assist_primal(D, idx, b, u,lam_di,lam_b,lam_up,rho_d,rho_up,horizon,  ...
...
%     u_this_for_paral{idx} = u{idx}(:,j);
    ...
    c(j) =  evaluateCost(D, idx, b_parallel,u_parallel,...
        lam_di(:,:,j),lam_b(:,:,j),lam_up(:,:,j),rho_d,rho_up,horizon, ...
        stateValidityChecker);
%     else
...
function cost = evaluateCost(D, idx, b, u, lam_di,lam_b,lam_up,rho_d,rho_up, ...
