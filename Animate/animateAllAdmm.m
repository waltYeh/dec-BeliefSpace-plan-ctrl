function [failed, b_f, x_true_final]...
    = animateAllAdmm(fig_xy,fig_w,D,agents, b0, x_true, nSteps,time_past, show_mode,draw_cov)
...
%     from_agent=5;
%     u{from_agent} = agents{from_agent}.getNextControl(b(from_agent,:));
%     for i=1:size(D.Nodes,1)
%         if i==from_agent
%         else
%             u{i} = agents{from_agent}.u_nom{i}(:,agents{from_agent}.ctrl_ptr-1);
%         end
%     end
...
%             u{i} = u{i}+u_w;
        ...
        elseif i>=5
            processNoise = agents{i}.motionModel.generateProcessNoise(x_true(i+1,:),u{i});
...
%         plot(mu_save{4,1}(1,k+1),mu_save{4,1}(2,k+1),'go')
    ...
    %         pointsToPlot = drawResult([mu_save{1,1}(:,k); sig_save{1,1}(:,k)], 2);
            ...
            plot(pointsToPlot(1,:),pointsToPlot(2,:),'b')
            pointsToPlot = drawResult([mu_save{6,1}(:,k+1); sig_save{6,1}(:,k+1)], 2);
            plot(pointsToPlot(1,:),pointsToPlot(2,:),'m')
        end
...
%         plot(z{1}(1),z{1}(2),'b*')
        ...
        plot([x_save(6,1,k-1),x_save(6,1,k)],[x_save(6,2,k-1),x_save(6,2,k)],'-r.')
        %     
...
plot(mu_save{6,1}(1,:),mu_save{6,1}(2,:),'go')
...
% if k>1
    ...
    plot(squeeze(x_save(6,1,:)),squeeze(x_save(6,2,:)),'-r.')
    figure(fig_w+1)
    subplot(2,2,1)
